...
function BuyerSetup() {
  ...
  async function handleSaveAccount() {
    ...
    let Tezos = await setup(accData.privateKey);
    let market = await Market.init(Tezos);
    let pkh = await Tezos.signer.publicKeyHash();
    let operation = await market.register(
      `${selectedPlanIndex}`,
  ...
  async function handleChangePlan() {
    ...
    let operation = await market.changeSubscription(selectedPlanIndex);
  }
  return (
            ...
            {isAccount ? (
              <Button
                className={classNames(
                  ...
                  JSON.parse(localStorage.getItem('buyerAccount')).plan ===
                    selectedPlanIndex
                    ? 'dark'
                    : 'purple'
                )}
